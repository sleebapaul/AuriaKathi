...
def parse_args():
    ...
    parser.add_argument('--input_text_folder',
                        help='Folder which contains haiku.txt which has the input text to convert into a image using AttnGAN.', 
                        dest='input_text_folder',
                        type=str)
...
def gen_example_from_text(input_text_folder, output_dir, wordtoix, algo):
    '''
    ...
    try:
        with open("{}/haiku.txt".format(input_text_folder), "r") as f:
            input_text = f.read()
            print(input_text)
    except FileNotFoundError as e:
        print("haiku.txt not found!")
        input_text = "never settle for less"
        print(input_text)
    if len(input_text) == 0:
        return 0
    ...
    if cfg.TRAIN.FLAG:
        ...
        if cfg.B_VALIDATION:
            ...
            gen_example_from_text(
                args.input_text_folder, args.output_dir, dataset.wordtoix, algo)
    end_t = time.time()
